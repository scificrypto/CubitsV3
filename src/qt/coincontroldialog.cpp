...
QString CoinControlDialog::getPriorityLabel(double dPriority)
{
    ...
    {
        if      (dPriority > 57600000000ULL)   return tr("highest");
        else if (dPriority > 5760000000ULL)    return tr("high");
        else if (dPriority > 576000000ULL)     return tr("medium-high");
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = CubitsUnits::BTC;
    ...
    l2->setText(CubitsUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(CubitsUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(CubitsUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));                                    // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText((fLowOutput ? (fDust ? tr("DUST") : tr("yes")) : tr("no"))); // Low Output / Dust
    l8->setText(CubitsUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    ...
    // tool tips
    l5->setToolTip(tr("This label turns red, if the transaction size is bigger than 10000 bytes.\n\n This means a fee of at least %1 per kb is required.\n\n Can vary +/- 1 Byte per input.").arg(CubitsUnits::formatWithUnit(nDisplayUnit, CENT)));
    l6->setToolTip(tr("Transactions with higher priority get more likely into a block.\n\nThis label turns red, if the priority is smaller than \"medium\".\n\n This means a fee of at least %1 per kb is required.").arg(CubitsUnits::formatWithUnit(nDisplayUnit, CENT)));
    l7->setToolTip(tr("This label turns red, if any recipient receives an amount smaller than %1.\n\n This means a fee of at least %2 is required. \n\n Amounts below 0.546 times the minimum relay fee are shown as DUST.").arg(CubitsUnits::formatWithUnit(nDisplayUnit, CENT)).arg(CubitsUnits::formatWithUnit(nDisplayUnit, CENT)));
    l8->setToolTip(tr("This label turns red, if the change is smaller than %1.\n\n This means a fee of at least %2 is required.").arg(CubitsUnits::formatWithUnit(nDisplayUnit, CENT)).arg(CubitsUnits::formatWithUnit(nDisplayUnit, CENT)));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            //calculate weight
            double nTimeWeight = min(GetTime() - out.tx->GetTxTime() - nStakeMinAgeV2, (int64)nStakeMaxAgeV2) ;
            ...
            {
                sAddress = CCubitsAddress(outputAddress).ToString().c_str();
            ...
            itemOutput->setText(COLUMN_AMOUNT, CubitsUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
            ...
            itemOutput->setText(COLUMN_AGE, strPad(CubitsUnits::formatAge(nDisplayUnit, age), 2, " "));
            itemOutput->setText(COLUMN_AGE_INT64, strPad(QString::number(age), 15, " "));
            
            // Potential Stake
            double nPotentialStake = min(3.0 / 365 * nBlockSize * nAge / (1440 * 60), 100.0); //min of the max reward or the stake rate
            ...
            itemOutput->setText(COLUMN_TIMEESTIMATE, strPad(CubitsUnits::formatAge(nDisplayUnit, nEstimateTime), 15, " ")); 
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, CubitsUnits::format(nDisplayUnit, nSum));
            itemWalletAddress->setText(COLUMN_AMOUNT_INT64, strPad(QString::number(nSum), 15, " "));
			itemWalletAddress->setText(COLUMN_POTENTIALSTAKE, CubitsUnits::formatAge(nDisplayUnit, nPotentialStakeSum));
