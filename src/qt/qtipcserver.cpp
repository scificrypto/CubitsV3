...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "Cubits:"))
        {
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        ...
        // Make sure we don't lose any Cubits: URIs
        for (int i = 0; i < 2; i++)
        {
        ...
        // Make sure only one Cubits instance is listening
        message_queue::remove(BOUNTYCOINURI_QUEUE_NAME);
        delete mq;
