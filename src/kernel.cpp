...
{	
	...
	if (nTimeTx > VERSION2_SWITCH_TIME)
        nTimeWeight = min((int64)nTimeTx - (int64)nTxPrevTime - GetStakeMinAge(nTimeTx), (int64)GetStakeMaxAge(nTimeTx));
    else
        nTimeWeight = min((int64)nTimeTx - (int64)nTxPrevTime, (int64)GetStakeMaxAge(nTimeTx)) - GetStakeMinAge(nTimeTx);
...
bool CheckStakeKernelHash(unsigned int nBits, const CBlock& blockFrom, unsigned int nTxPrevOffset, const CTransaction& txPrev, const COutPoint& prevout, unsigned int& nTimeTx, bool fCheck, uint256& hashProofOfStake, bool fPrintProofOfStake)
{
	...
	unsigned int i;
	for(i = 0; i < 60 && !fSuccess; i++) //iterate the hashing
	{
        //hash this iteration
		nTryTime = nTimeTx + 60 - i;
		hashProofOfStake = stakeHash(nTryTime, nTxPrevTime, ss, prevout.n, nTxPrevOffset, nTimeBlockFrom); 

		// if stake hash does not meet the target then continue to next iteration
		if(!stakeTargetHit(hashProofOfStake, (int64)nTryTime, nTxPrevTime, nValueIn, bnTargetPerCoinDay))
			continue;
...
bool CheckProofOfStake(const CTransaction& tx, unsigned int nBits, uint256& hashProofOfStake)
{
    ...
    if (!CheckStakeKernelHash(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, txPrev, txin.prevout, nTxTime, true, hashProofOfStake, fDebug))
